...
function sparkleCount(base: number, q: GraphicsQuality): number {
  if (q === 'medium') return Math.ceil(base * 0.5);
  return base;
}
...
export function GameEffects() {
  ...
  const sc = useMemo(
    () => (base: number) => sparkleCount(base, graphicsQuality),
    [graphicsQuality],
  );
  ...
  return (
    <group position={[0, 0.15, -2.8]}>
      {!reducedMotion && !skipSparkles && worldParticleBurst === 'confetti' ? (
      ...
      {!reducedMotion && !skipSparkles && worldParticleBurst === 'levelup' ? (
        <group position={[0, 4, 8]}>
          <Sparkles count={sc(48)} scale={3.4} size={3} speed={0.5} opacity={0.92} color={new Color(0xfde047)} />
          <Sparkles count={sc(36)} scale={2.8} size={2.2} speed={0.42} opacity={0.82} color={new Color(0xf97316)} />
        </group>
      ) : null}
      {!reducedMotion && !skipSparkles && xpBurst ? (
        <Sparkles
          count={sc(32)}
          scale={2.4}
          size={2.4}
      ...
      {!reducedMotion && !skipSparkles && levelBurst ? (
        <group position={[0, 3.35, 2.15]}>
          <Sparkles count={sc(55)} scale={3.6} size={3.2} speed={0.55} opacity={0.92} color={new Color(0xfde047)} />
          <Sparkles count={sc(40)} scale={3} size={2.2} speed={0.45} opacity={0.8} color={new Color(0xf97316)} />
        </group>
      ) : null}
    </group>
  );
